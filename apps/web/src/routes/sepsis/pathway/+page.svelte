<script lang="ts">
	...
	import { sepsisPathway } from '$lib/data/trees/sepsis-pathway';
	let activeView: 'tree' | 'textbook' | 'split' = 'split';
</script>
<svelte:head><title>Sepsis First-Hour Bundle — PED CDS</title></svelte:head>
<div class="page">
	...
	<div class="content" class:split={activeView === 'split'}>
		{#if activeView !== 'textbook'}<div class="tree"><ClinicalDecisionTree tree={sepsisPathway} /></div>{/if}
		{#if activeView !== 'tree' && sepsisPathway.textbook}<div class="text"><TextbookViewer section={sepsisPathway.textbook} /></div>{/if}
	</div>
</div>
<style>
