<script lang="ts">
	...
	import { rsiProtocol } from '$lib/data/trees/rsi-protocol';
	let activeView: 'tree' | 'textbook' | 'split' = 'split';
</script>
<svelte:head><title>Rapid Sequence Intubation — PED CDS</title></svelte:head>
<div class="page">
	...
	<div class="content" class:split={activeView === 'split'}>
		{#if activeView !== 'textbook'}<div class="tree"><ClinicalDecisionTree tree={rsiProtocol} /></div>{/if}
		{#if activeView !== 'tree' && rsiProtocol.textbook}<div class="text"><TextbookViewer section={rsiProtocol.textbook} /></div>{/if}
	</div>
</div>
<style>
